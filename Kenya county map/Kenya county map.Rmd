...
Kisumu, Siaya, and Homa Bay highlighted

```{r Kenya county map}

mapcolors <- c("seagreen", "coral", "goldenrod")
...
kenya <- st_read("/Users/tconcepc/Downloads/kenyan-counties/County.shp")
lake_victoria <- st_read("/Users/tconcepc/Downloads/Lake_Victoria_Shapefile/Lake_Victoria_Shapefile.shp")
...
## Plot map
set.seed(143) # Set reproducibe seed, since we're using geom_label_repel
ggplot(data = kenya) +
  geom_sf(fill = "gray95", color = "black") +  # Base map
  geom_sf(data = highlighted, fill = mapcolors, color = "black") +  # Highlighted counties
  geom_sf(data = lake_victoria_kenya, fill = "skyblue", color = "black") +  # Lake Victoria overlay
  geom_label_repel(data = centroids, 
                   ...
                   size = 4, 
                   fontface = "bold", 
                   box.padding = .5, # Amount to "repel" the label
                   min.segment.length = .5, # Should match the padding above, if you want the line to connect
                   segment.size = 1, # Makes the line to the label thicker
